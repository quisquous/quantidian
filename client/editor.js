...
function editorAddQuestion() {
  ...
  if (!editor.questions || !editor.questions.length) {
    ...
    new_num = _.max(editor.questions, function(q) {
      return q.num;
    }).num + 1;
  }
...
function editorAddChoice(num) {
  var editor = Session.get('editor');
  var q = _.find(editor.questions, function(q) {
    return q.num == num;
  });
  ...
  if (!q.choices || !q.choices.length) {
    ...
    new_num = _.max(q.choices, function(c) {
      return c.num;
    }).num + 1;
  }
...
function editorUpdateChoice(question_num, choice_num, key, value) {
  var editor = Session.get('editor');
  var q = _.find(editor.questions, function(q) {
    return q.num == question_num;
  });
  var c = _.find(q.choices, function(c) {
    return c.num == choice_num;
  });
  c[key] = value;
  Session.set('editor', editor);
}
...
Template.editor.events({
  ...
  'click .savecategory': function(evt, template) {
    ...
    _.each(editor.questions, function(q) {
      delete q.num;
    });
    _.each(editor.questions.choices, function(c) {
      delete c.num;
    });
...
Template.editorQuestion.events({
  ...
  'click .deletechoice': function(evt, template) {
    var editor = Session.get('editor');
    var q = _.find(editor.questions, function(q) {
      return q.num == template.data.num;
    });
  ...
  'change .choicetext': function(evt, template) {
    editorUpdateChoice(template.data.num, choiceNumForElement(evt.target),
